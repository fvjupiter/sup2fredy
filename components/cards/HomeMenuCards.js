...
export default function HomeMenuTiles() {
  ...
  const navItems = [
    [
      continueAt.writings,
      "Writings",
      <Icon key={"0"} id={"writings"} size={70} />,
      "border-lime-400",
      "text-lime-400",
      continueAt.writings.includes("poems")
        ? bg_writings.poems
        : continueAt.writings.includes("notes")
        ? bg_writings.notes
        : continueAt.writings.includes("stories")
        ? bg_writings.stories
        : bg_writings.menu,
      "Poems, Notes & Stories",
    ],
    ...
    [
      continueAt.software,
      "Software",
      <Icon key={"2"} id={"software"} size={70} />,
      "border-pink-400",
      "text-pink-400",
      bg_software.menu,
      "Click-Games",
        ...
        {navItems.map((item, index) => (
          <NavItem
            ...
            ifClickedScale={
              index == isClicked
                ? "scale-[2]"
                : "scale-[1.08] group-hover:scale-[1]"
            }
...
const NavItem = ({
  ...
  return (
    <Link href={href}>
      ...
      >
        <Image
          className={`opacity-50 group-hover:opacity-100 transform md:group-active:scale-[2] ${ifClickedScale} transition-all duration-300`}
          src={bgImg}
          placeholder="blur"
          layout="fill"
          objectFit="cover"
        ...
        />
        <div
          className={`absolute w-full mr-2 border-t-2 ${border} ${ifClickedOpacity} md:group-active:opacity-0 textShadow text-white transition-all duration-100 z-10 backdrop bg-opacity-60 bg-black px-3 py-1 flex items-center justify-between`}
        >
          <div className="text-md sm:text-lg font-extralight">{title}</div>
          <div className="text-sm sm:text-lg font-extralight">{content}</div>
        </div>
        <div
