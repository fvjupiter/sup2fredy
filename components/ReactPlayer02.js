...
export default function AudioPlayer({ trackList, width, height }) {
  ...
  return <div className='mx-auto max-w-max mt-4 bg-black rounded-2xl overflow-hidden bigShadow mb-20'>
        ...
        <div className='text-white'>
            {trackList.map((url, index) => <div key={index} onClick={() => { setid(index); setisPlaying(true) }} className={`px-4 py-1 ${index == trackList.length-1 && 'pb-2'} ${index == id ? 'bg-orange-500 text-white' : 'hover:bg-blueGray-800 button-hover-white'} cursor-pointer`}>{url.title}</div>)}
        </div>
  </div>
}
