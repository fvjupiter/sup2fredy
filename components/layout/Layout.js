...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    if (showInstaAlert && navigator.userAgent.includes("Instagram")) {
      ...
      alert(
        "Dear Instagram-User, it is recommended to open the app in an external browser at\nsup2fredy.art\n\n💖\n\nSincerely, Fredy"
      );
    }
  }, [asPath]);
  ...
  useEffect(() => {
    setcontinueAt({
      ...continueAt,
      [`${asPath.split("/")[1] === "" ? "home" : asPath.split("/")[1]}`]:
        asPath,
    });
    if (!isShowMenu) {
    ...
    <>
      ...
      <FullScreen handle={handleFullscreen}>
        ...
        <Page isLandingPage={isLandingPage} setisImprint={setisImprint}>
          {children}
          {trackList && (
            <div className={`${param1 == "music" ? "visible" : "hidden"}`}>
  ...
  return (
    <div
      ...
      className={`fixed top-0 left-0 right-0 bottom-0 ${
        !isLandingPage ? "overflow-scroll" : "overflow-hidden"
      } z-10`}
    >
      <ScrollToTop />
