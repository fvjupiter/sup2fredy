...
export default function Home() {
  ...
  return (
    <>
      <div ref={top1} className="absolute top-0" />
      {displayMagic && screen.width > 110 && (
        <div
          className={`fixed top-0 z-90 flex justify-center items-center bg-black w-full ${
            isLandingPage ? "opacity-100 h-full" : "opacity-0 h-0"
          }`}
        >
          <div className="">
      ...
      <div
        className={`w-full md:w-10/12 max-w-4xl mx-auto px-1 transition-opacity duration-1000 ${
          !isLandingPage ? "opacity-100 scale-100" : "opacity-0 scale-0"
        }`}
      >
        <h1 className="text-4xl sm:text-6xl xl:text-7xl font-cursive textShadow-lg text-white font-extrabold mx-auto w-max my-3 sm:mb-6 sm:mt-8 xl:mb-10 xl:mt-12">
          SUP2FREDY
        </h1>
        <HomeMenuTiles />
        <div className="mb-20 center px-4">
          <div className="text-white text-sm backdrop bg-opacity-60 bg-black w-fit p-1">
            Click &apos;add to Homescreen&apos; in your Web-Browser to install
            this Blog as an App (PWA).
          </div>
        </div>
