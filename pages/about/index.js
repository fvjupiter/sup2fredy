...
export default function About({ aboutData }) {
  ...
  return (
    <div className="">
      <div
        className={`${
          scrollTop > screen.height - 260 ? "opacity-0" : "opacity-100"
        } fixed w-screen text-center z-40 duration-1000`}
      >
      ...
      <div
        className={`${
          isBigImg
            ? "scale-100 rotate-0 ml-0"
            : "ml-[440px] sm:ml-0 sm:scale-0 sm:rotate-[540deg]"
        }
      ...
      <div className={`h-screen absolute w-screen z-40`}>
        <div
          style={{ marginTop: screen.height - 175 }}
          className={`w-screen text-white z-10 mb-20 px-1.5`}
        >
          <div
            className={`whitespace-pre-line lg:w-[1010px] mx-auto sm:w-[620px] md:w-[748px]
      ...
      <div
        style={{ opacity: screen.width < 768 ? scrollTop / 700 : null }}
        className={`${
          scrollTop > 0 ? "md:opacity-100" : "md:opacity-0"
        } left-0 right-0 bottom-0 top-0 w-screen h-screen fixed z-30 md:duration-[7s]`}
      >
        <Image
          src={RainbowFractal}
          placeholder="blur"
